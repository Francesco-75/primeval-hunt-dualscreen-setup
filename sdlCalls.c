...
void startSDL(int *argcp, char **argv)
{
    ...
    // Per Primeval Hunt con fullscreen usiamo una finestra borderless —
    // SDL_SetWindowFullscreen non funziona correttamente con TwinView nvidia
    // e SDL_VIDEO_X11_XRANDR=0. La finestra viene poi spostata a 0,0
    // con dimensioni 1280x480 tramite Xlib dopo SDL_ShowWindow.
    if (gId == PRIMEVAL_HUNT && getConfig()->fullscreen)
    ...
    if (gId == PRIMEVAL_HUNT && getConfig()->fullscreen)
    {
        // Non usiamo SDL_SetWindowFullscreen che va sul display fisico
        // sbagliato. La finestra borderless viene posizionata tramite
        // Xlib dopo SDL_ShowWindow.
        isFullScreen = true;
    }
    else if (getConfig()->fullscreen)
    {
    ...
    // Per Primeval Hunt: dopo ShowWindow la finestra è mappata.
    // Con TwinView nvidia e SDL_VIDEO_X11_XRANDR=0 il framebuffer è
    // un unico display 1280x480 — XMoveResizeWindow può espandersi
    // liberamente senza che Mutter blocchi il resize.
    if (gId == PRIMEVAL_HUNT && getConfig()->fullscreen && x11Display && x11Window)
    {
        XMoveResizeWindow(x11Display, x11Window, 0, 0, gWidth, gHeight);
        XRaiseWindow(x11Display, x11Window);
        XFlush(x11Display);
        usleep(100000);
        // Secondo tentativo per sicurezza
        XMoveResizeWindow(x11Display, x11Window, 0, 0, gWidth, gHeight);
        XFlush(x11Display);
    }
